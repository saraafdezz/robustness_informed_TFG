...
participant_df = pd.DataFrame(
    [line.split("\t") for line in participant_data[1:]],
    columns=["PARTICIPANT", "PARTICIPANT_TYPE", "PARTICIPANT_NAME", "UNIFICATION_XREF"],
)
...
for chunk in tqdm(
    ...
    def map_participant(x):
        ...
        parts = str(x).split("*")
        mapped = [
            name_mapping.get(p, p) for p in parts if p
    ...
    mask = (chunk["PARTICIPANT_A"].str.len() > 0) | (
        chunk["PARTICIPANT_B"].str.len() > 0
    )
    chunk = chunk[mask]

    # Process each chunk.
    grouped_chunk = (
        chunk.groupby(["PATHWAY_NAMES", "INTERACTION_TYPE", "MEDIATOR_IDS"])
        .agg(
            {
...
print("Grouping data.")
final_grouped = (
    final_df.groupby(["PATHWAY_NAMES", "INTERACTION_TYPE", "MEDIATOR_IDS"])
    .agg(
        {
            "PARTICIPANT_A": lambda x: "*".join(
                sorted(set(x.str.split("*").explode()))
            ),
            "PARTICIPANT_B": lambda x: "*".join(
                sorted(set(x.str.split("*").explode()))
            ),
        }
    )
    .reset_index()
    .drop("MEDIATOR_IDS", axis=1)
)
...
mask = final_grouped["PATHWAY_NAMES"].str.contains(
