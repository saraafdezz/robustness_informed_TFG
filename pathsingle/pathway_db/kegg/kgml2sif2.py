    ...
    for kgml_file in os.listdir(kgml_dir):
        if kgml_file.endswith(".kgml"):
            ...
            for relation in pathway.relations:
                ...
                out.write(
                    f"{kgml_file.replace('.kgml','')},{entry1_names},{subtypes},{entry2_names}\n"
                )
...
pathway_relations_df["source"] = pathway_relations_df["source"].apply(
    replace_ids_with_symbols
)
pathway_relations_df["target"] = pathway_relations_df["target"].apply(
    replace_ids_with_symbols
)

# Drop rows where both 'source' and 'target' are empty strings.
pathway_relations_df = pathway_relations_df[
    ~((pathway_relations_df["source"] == "") & (pathway_relations_df["target"] == ""))
]
pathway_relations_df = pathway_relations_df[
    (pathway_relations_df["interactiontype"] != "")
    & (~pathway_relations_df["interactiontype"].isnull())
]
