...
@flow(
    name="m-IVAE",
    task_runner=RayTaskRunner(init_kwargs={"log_to_driver":False, "logging_config":ray.LoggingConfig(encoding="JSON", log_level="ERROR")}),
)
def main(
    ...
    for model in models:
        ...
        for seed in seeds:
            with remote_options(num_cpus=1, num_gpus=1):
                result_future = fit_model.submit(
                        ivae_config, seed, data_path, debug
                    )
            
            with remote_options(num_cpus=n_cpus_clustering, num_gpus=0):
                clustering_metrics_futures = evalute_clustering.submit(result_future, seed)
        ...
        with remote_options(num_cpus=10, num_gpus=0):
            results_by_model_futures = gather_results.submit(ivae_config, results)
            consistedness_futures = compute_consistedness.submit(results_by_model_futures)
            results_by_model.append(results_by_model_futures)
            consistedness.append(consistedness_futures)

    # Wait for all tasks to finish
    wait(clustering_metrics)
    wait(results_by_model)
    wait(consistedness)
