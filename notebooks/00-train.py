...
if __name__ == "__main__":
    ...
    parser.add_argument("--debug", type=bool, help="True to debug the model with less epochs")
    
    parser.add_argument("--adjacency_names", help = "vector or name with names of adjacency matrix to use")
    
    parser.add_argument("--adjacency_activation", help = "vector or name with activation functions for each layer of adjacency matrix")
    
    parser.add_argument("--seed", type=int,default=42, help="Type of model")
    
    parser.add_argument("--frac", type=int,default=1, help="Distribution of random layer")
    ...
    random_layer, random_layer_names = get_random_adj(
        frac, shape=reactome.shape, size=reactome.size, index=reactome.index, seed=args.seed)
...
#         x_trans, random_layer = sync_gexp_adj(x_trans, random_layer)
    ...
    model = InformedVAE(adjacency_matrices=reactome,adjacency_names= args.adjacency_names,adjacency_activation=args.adjacency_activation)
    model._build_vae()
    batch_size = 32
    ...
    pd.DataFrame.from_dict(evaluation).reset_index(names="metric").assign(seed=args.seed).melt(
        id_vars=["seed", "metric"],
